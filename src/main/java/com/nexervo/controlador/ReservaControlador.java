...
public class ReservaControlador implements PrincipalControlador.NecesitaUsuario {
    ...
    public void initialize() {
        ...
        configurarOcasion();
        cargarMapa();
        fechaReserva.valueProperty().addListener((o,v,n) -> { limpiarError(fechaReserva); actualizarHoras(n); actualizarMapa(); });
        comboHora.valueProperty().addListener((o,v,n) -> { limpiarError(comboHora); actualizarMapa(); });
        // Limpiar borde rojo al escribir en campos obligatorios
        txtNombre.textProperty().addListener((o,v,n)    -> limpiarError(txtNombre));
        txtApellidos.textProperty().addListener((o,v,n) -> limpiarError(txtApellidos));
        txtTelefono.textProperty().addListener((o,v,n)  -> limpiarError(txtTelefono));
    ...
    private boolean validar() {
        ...
        boolean ok = true;
        if (txtNombre.getText().isBlank())    { marcarError(txtNombre);    ok = false; }
        if (txtApellidos.getText().isBlank()) { marcarError(txtApellidos); ok = false; }

        // El teléfono tiene que tener exactamente 9 dígitos (el listener ya filtra letras)
        String tel = txtTelefono.getText().trim();
        if (tel.isBlank() || tel.length() != 9) { marcarError(txtTelefono); ok = false; }

        if (fechaReserva.getValue() == null)  { marcarError(fechaReserva); ok = false; }
        if (comboHora.getValue() == null || comboHora.getValue().startsWith("──")) { marcarError(comboHora); ok = false; }
        if (idMesaSeleccionada == -1)         { marcarError(txtMesaSeleccionada); ok = false; }

        // Email opcional, pero si se rellena tiene que tener formato válido
        ...
        if (!ok) {
            alerta(Alert.AlertType.WARNING, "Completa los campos marcados en rojo antes de continuar.");
            if (txtNombre.getText().isBlank())              txtNombre.requestFocus();
            else if (txtApellidos.getText().isBlank())      txtApellidos.requestFocus();
            else if (tel.isBlank() || tel.length() != 9)   txtTelefono.requestFocus();
        }
        return ok;
    }
    ...
    private void marcarError(javafx.scene.Node campo) {
        if (campo instanceof javafx.scene.control.ComboBox) campo.getStyleClass().add("combo-invalido");
        else if (campo instanceof javafx.scene.control.DatePicker) campo.getStyleClass().add("date-picker-invalido");
        else campo.getStyleClass().add("campo-invalido");
    }

    private void limpiarError(javafx.scene.Node campo) {
        campo.getStyleClass().removeAll("campo-invalido", "combo-invalido", "date-picker-invalido");
    }

    private void limpiarTodosLosErrores() {
        for (javafx.scene.Node n : new javafx.scene.Node[]{
                txtNombre, txtApellidos, txtTelefono, txtEmail,
                fechaReserva, comboHora, txtMesaSeleccionada}) {
            if (n != null) limpiarError(n);
        }
    }
